...
- Local run evidence checked by this repo with `python scripts/export_readme_assets.py --run-dir outputs/act_pusht_baseline --out-dir images`.
...
These images are used only as ecosystem references. The runnable evidence claimed by LunaVLA comes from the local PushT-style loop, generated reports, rollout browser, and checked README media manifest in this repository.
