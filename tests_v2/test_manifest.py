...
def _valid_manifest(tmp_path: Path) -> RunManifest:
    checkpoint = tmp_path / "strict-checkpoint.json"
    ...
    return RunManifest.create(
        root=Path.cwd(),
        config=_config("outputs/strict-manifest"),
        data_sha256=sha256_transitions(transitions),
        checkpoint_path=checkpoint,
        dataset_split={"train": [0], "validation": [], "test": []},
        command=["lunavla-v2", "train"],
        metrics={"success_rate": 0.5, "distance": {"mean": 0.25}},
        paired_data={"donor_bank_sha256": "d" * 64},
        arms=[{"id": "control"}],
        pairs=[],
    )
...
@pytest.mark.parametrize(
    ("mutation", "message"),
    [
        (lambda payload: payload.__setitem__("data_sha256", "A" * 64), "data_sha256"),
        (
            lambda payload: payload.__setitem__("checkpoint_sha256", "short"),
            "checkpoint_sha256",
        ),
        (
            lambda payload: payload.__setitem__(
                "artifact_sha256", {"metrics.json": "z" * 64}
            ),
            "artifact_sha256.metrics.json",
        ),
        (
            lambda payload: payload["paired_data"].__setitem__(
                "donor_bank_sha256", "bad"
            ),
            "paired_data.donor_bank_sha256",
        ),
        (
            lambda payload: payload.__setitem__("eval_fixture_sha256", "bad"),
            "eval_fixture_sha256",
        ),
        (lambda payload: payload.__setitem__("git_dirty", "false"), "git_dirty"),
        (
            lambda payload: (
                payload.__setitem__("git_dirty", False),
                payload.__setitem__("source_diff_sha256", "a" * 64),
            ),
            "clean manifests",
        ),
    ],
)
def test_schema3_strictly_validates_hash_and_source_state_contracts(
    tmp_path: Path,
    mutation: object,
    message: str,
) -> None:
    ...
    path = tmp_path / "invalid-hash-manifest.json"
    path.write_text(json.dumps(payload), encoding="utf-8")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda payload: payload.__setitem__("train_seeds", [5, 5]), "duplicate"),
        (lambda payload: payload.__setitem__("eval_seeds", [999]), "eval_seeds"),
        (
            lambda payload: payload["dataset_split"].__setitem__("extra", []),
            "exactly train",
        ),
        (
            lambda payload: payload["dataset_split"].__setitem__("test", [0]),
            "multiple splits",
        ),
        (
            lambda payload: payload.__setitem__("dependencies", {"numpy": 2}),
            "dependencies.numpy",
        ),
        (lambda payload: payload.__setitem__("command", "train"), "command"),
        (
            lambda payload: payload.__setitem__("metrics", {"loss": float("nan")}),
            "finite",
        ),
    ],
)
def test_schema3_strictly_validates_seeds_split_dependencies_command_and_metrics(
    tmp_path: Path,
    mutation: object,
    message: str,
) -> None:
    ...
    payload = manifest.to_dict()
    mutation(payload)  # type: ignore[operator]
    path = tmp_path / "invalid-typed-manifest.json"
    path.write_text(json.dumps(payload), encoding="utf-8")
    with pytest.raises((TypeError, ValueError), match=message):
        RunManifest.load(path)


def test_manifest_is_deeply_read_only_but_to_dict_is_mutable_and_round_trips(
    tmp_path: Path,
) -> None:
    manifest = _valid_manifest(tmp_path)
    path = manifest.write(tmp_path / "immutable-manifest.json")
    loaded = RunManifest.load(path)

    with pytest.raises(TypeError, match="read-only"):
        loaded.config["policy"]["type"] = "tampered"
    with pytest.raises(TypeError, match="read-only"):
    ...
    exported["eval_seeds"].append(999)
    assert loaded.config["policy"]["type"] == "numpy_linear_chunk"
    assert loaded.eval_seeds == [90, 91]
    assert RunManifest.load(path).to_dict() == loaded.to_dict()


def test_schema2_requires_exact_legacy_root_and_to_dict_preserves_it(
    tmp_path: Path,
) -> None:
    payload = _valid_manifest(tmp_path).to_dict()
    payload["schema_version"] = 2
    ...
    for name in schema3_only:
        payload.pop(name)
    path = tmp_path / "schema2-exact.json"
    path.write_text(json.dumps(payload), encoding="utf-8")
    legacy = RunManifest.load(path)
    assert set(legacy.to_dict()) == set(payload)

    payload.pop("metrics")
    path.write_text(json.dumps(payload), encoding="utf-8")
    with pytest.raises(ValueError, match="missing manifest field.*metrics"):
        RunManifest.load(path)
