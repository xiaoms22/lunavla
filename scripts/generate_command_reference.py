...
def build_reference() -> str:
    ...
    lines: list[str] = [
        ...
        "5. Run `python scripts/generate_policy_ladder.py` after BC smoke and baseline evidence exist.",
        "6. Run `python scripts/generate_action_chunk_lesson.py` again after the baseline checkpoint exists.",
        "7. Run `python scripts/run_ablation_evidence.py` after the baseline report exists.",
        "8. Finish with `python scripts/build_evidence_pack.py --skip-runs` and `python scripts/build_submission_pack.py`.",
        "",
        "## Command Map",
        "",
    ]
