...
def build_reference() -> str:
    ...
    lines: list[str] = [
        ...
        "8. Run `python scripts/run_jsonl_data_smoke.py` when you want the first stronger-data extension.",
        "9. Run `python scripts/run_data_quality_comparison.py` to compare clean vs noisy local JSONL demonstrations.",
        "10. Run `python scripts/run_policy_tuning_comparison.py` for a BC hidden-size comparison.",
        "11. Run `python scripts/generate_policy_ladder.py` after BC smoke and baseline evidence exist.",
        "12. Run `python scripts/generate_action_chunk_lesson.py` again after the baseline checkpoint exists.",
        "13. Run `python scripts/run_ablation_evidence.py` after the baseline report exists.",
        "14. Finish with `python scripts/build_evidence_pack.py --skip-runs` and `python scripts/build_submission_pack.py`.",
        "",
        "## Command Map",
        "",
    ]
