...
COMMANDS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "purpose": "Read saved rollout traces, count failed phases/subtasks, and assign first-pass task labels.",
        "outputs": [
            "outputs/task_understanding_report.md",
            "outputs/task_understanding_report.csv",
        ],
        "next": "Inspect saved rollout JSON before changing task labels or tuning policies.",
    },
    {
    ...
    },
    {
        ...
        "purpose": "Compare two BC hidden sizes with the same dataset and evaluation shape.",
        "outputs": [
        ...
        "next": "Use the comparison to explain why policy tuning still needs rollout evidence.",
    },
    {
        "stage": "policy ladder",
    ...
    },
    {
        ...
        "purpose": "Compare train-time action targets with eval-time executable rollout actions across runs.",
        "outputs": [
            "outputs/action_analysis_report.md",
            "outputs/action_analysis_report.csv",
        ],
        "next": "Inspect clipped fractions and action scale before adding an action-normalization ablation.",
    },
    {
        "stage": "extended evaluation",
        "command": "python scripts/run_extended_evaluation.py",
        "purpose": "Rerun more rollout episodes, save rollout demos, and compare success rate with final distance.",
        "outputs": [
        ...
        "next": "Open one success example and one failure example before writing an evaluation conclusion.",
    },
    {
        "stage": "jsonl data",
        "command": "python scripts/export_pusht_jsonl_dataset.py",
        "purpose": "Export a small PushT-style demonstration set as local JSONL.",
        "outputs": [
            "data/examples/pusht_jsonl_smoke.jsonl",
            "outputs/jsonl_dataset_export.md",
        ],
        "next": "Reload the file with the JSONL smoke config.",
    },
    {
        "stage": "jsonl data",
        "command": "python scripts/run_jsonl_data_smoke.py",
        "purpose": "Train, evaluate, and report a policy from `dataset.source: jsonl`.",
        "outputs": [
            "data/examples/pusht_jsonl_smoke.jsonl",
            "outputs/jsonl_dataset_inspection.md",
            "outputs/act_pusht_jsonl_smoke/summary_report.md",
            "outputs/act_pusht_jsonl_smoke/project_report.md",
            "outputs/act_pusht_jsonl_smoke/run_diagnostic.md",
            "outputs/jsonl_action_statistics.md",
        ],
        "next": "Use this as the first stronger-data extension after the baseline works.",
    },
    {
        "stage": "data quality",
        "command": "python scripts/run_data_quality_comparison.py",
        "purpose": "Compare clean and noisier local JSONL demonstrations with the same policy/eval shape.",
        "outputs": [
            "data/examples/pusht_jsonl_smoke.jsonl",
            "data/examples/pusht_jsonl_noisy_smoke.jsonl",
            "outputs/data_quality_comparison.md",
            "outputs/data_quality_comparison.csv",
            "outputs/act_pusht_jsonl_smoke/project_report.md",
            "outputs/act_pusht_jsonl_noisy_smoke/project_report.md",
        ],
        "next": "Use this report to explain data quality without claiming real-robot performance.",
    },
    {
        "stage": "smoke",
    ...
    {
        "stage": "homepage summary",
        "command": "python scripts/generate_homepage_summary.py",
        "purpose": "Generate the README result card and short homepage evidence summary.",
        "outputs": [
            "outputs/homepage_summary.md",
            "images/homepage_results.svg",
        ],
        "next": "Use the homepage summary to keep README result claims tied to checked metrics.",
    },
    {
    ...
    },
    {
        ...
        "purpose": "Verify that public commands, generated artifacts, boundaries, and reviewer-facing claims are ready.",
        "outputs": ["outputs/reviewer_readiness.md"],
...
def build_reference() -> str:
    ...
    lines: list[str] = [
        ...
        "5. Run `python scripts/generate_task_understanding_report.py` to inspect phase/subtask behavior in saved rollouts.",
        "6. Run `python scripts/generate_action_statistics.py` to inspect action scale.",
        "7. Run `python scripts/generate_action_analysis_report.py` to compare train-time and eval-time actions.",
        "8. Run `python scripts/run_extended_evaluation.py` to save more rollout demos and inspect success/failure examples.",
        "9. Run `python scripts/generate_homepage_summary.py` to refresh the README result card.",
        "10. Run `python scripts/run_jsonl_data_smoke.py` when you want the first stronger-data extension.",
        "11. Run `python scripts/run_data_quality_comparison.py` to compare clean vs noisy local JSONL demonstrations.",
        "12. Run `python scripts/run_policy_tuning_comparison.py` for a BC hidden-size comparison.",
    ...
    lines.extend(
        [
            ...
            "- To explain task phases and failed subtasks: read `outputs/task_understanding_report.md`.",
            "- To explain action scale and normalization: read `outputs/action_statistics.md`.",
            "- To compare train/eval action scale: read `outputs/action_analysis_report.md`.",
            "- To explain stronger eval: read `outputs/extended_evaluation_report.md`.",
            "- To explain homepage claims: read `outputs/homepage_summary.md`.",
            "- To explain local file data: read `outputs/jsonl_dataset_export.md` and `outputs/act_pusht_jsonl_smoke/project_report.md`.",
            "- To explain data quality: read `outputs/data_quality_comparison.md`.",
            "- To explain policy tuning: read `outputs/policy_tuning_comparison.md`.",
